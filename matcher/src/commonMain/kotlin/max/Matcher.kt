...
interface Matcher {
    ...
    fun append(path: String): Matcher
    ...
    private class Impl(
        ...
        override fun match(path: String): Map<String, Any> {
            ...
            val map = mutableMapOf<String, Any>(SPLAT to emptyList<String>())
            for ((i, groupValue) in groupValues.withIndex()) {
                val name = names[i]
                if (name == SPLAT) {
                    @Suppress("UNCHECKED_CAST")
                    map[SPLAT] = (map[SPLAT] as List<String>) + groupValue
                } else {
                    map[name] = groupValue
                }
            }
        ...
        override fun append(path: String): Matcher {
        ...
        companion object {
            ...
            private fun pctEncode(c: Char): String {
                ...
                for (char in encoded) {
                    sb.append('%')
                    sb.append(char.toString(16).toUpperCase())
                }
                return sb.toString()
            }
    ...
    companion object {
        ...
        fun create(path: String): Matcher {
            return Impl(emptyList(), Regex("")).append(path)
        }
        ...
        fun splat(params: Map<String, Any>): List<String> {
            @Suppress("UNCHECKED_CAST")
            return params[SPLAT] as? List<String>
